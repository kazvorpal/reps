...
<html lang="en">
  ...
  <script>
   ...
   $(document).ready(function(){
        ...
        window.subprograms = subprograms;
        </script> 
...
let  formDataOld = {
    "mode": "global",   // global, project, program
    "type": "risk",     // risk, issue
    "format": "edit",  // create, edit, view
    "name": "Create Global Risk or Issue",  // free form string
    "description": "Enter the details of your Risk/Issue",  // free form string
    "fields": {
        "type": {
            "title": "Risk Issue Type",
            "type": "radio",        // radio, select, checkbox, textarea, datepicker, text
            "required": true,       // boolean
            "list": "types",    // I need to pass an array for dropdowns, checkbox lists, et cetera, may be an object where the keys are strings
            "values": "risk"    // empty, or a single value, or a comma-delimited list of values
        }, 
        "program": {
            "title": "Program",
            "type": "select",        
...
for (let count = -1; count < 2; count++) 
  ...
  const capitalize = (target) => target.trim().charAt(0).toUpperCase() + target.slice(1);
  // Sanitize a string
...
function Form({ formData }) {
  const { mode, type, format, name, description, fields } = formData;
    ...
    <div>
      ...
      <form className="field-container">
        {Object.keys(fields).map((key) => {
          ...
          return <Field key={key} fieldData={field} />;
        })}
      </form>
    </div>
  );
...
function Field({ fieldData }) {
  const { title, type, required, list, values, property } = fieldData;
  console.log({list});

  let field;
  switch (type) {
    case 'radio':
      field = list.map((value, index) => (
        <label  for={value} key={index}>
          <input type="radio" required={required} name={title} value={value} />
          {" "}{capitalize(value)}{" "}
        </label>
      ));
      break;
    case 'checkbox':
      field = Object.entries(list).map((value, index) => (
        <label for={value} key={index}>
          <input type="checkbox" required={required} name={title} value={value} />
          {" "}{value}{" "}
        </label>
      ));
      break;
      ...
      field = (
        <select required={required} name={title}>
          {list.map((value) => (
            <option value={(typeof value == "object") ? value.Program_Nm : value}>
              {(typeof value == "object") ? value.Program_Nm : value}
            </option>
          ))}
        </select>
      );
    ...
    case 'textarea':
    case 'datepicker':
      field = <input type={type} required={required} name={title} />;
      break;
    default:
      field = <input type="text" required={required} name={title} />;
    }
  ...
  const levels = ["program", "portfolio"];

  const formData = {
    mode: 'global',
    type: 'risk',
    format: 'edit',
    name: 'Create Global Risk or Issue',
    description: 'Enter the details of your Risk/Issue',
    fields: {
      lrp: {
        title: 'LRP Year',
        type: 'select',
      ...
      subprogram: {
        ...
        type: 'select',
        required: true,
        list: window.subprograms,
        property: "SubProgram_Nm",
      },
      descriptor: {
        title: 'Descriptor',
        type: 'text',
